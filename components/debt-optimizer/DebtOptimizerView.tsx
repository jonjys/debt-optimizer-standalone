  ...
  return (
    <div className="min-h-screen bg-[#09090B] text-[#FAFAFA] p-4 md:p-6 overflow-x-hidden">
      <div className="max-w- mx-auto">
        <h1 className="text- font-bold mb-1">Lånekalkylator</h1>
        <p className="text-[#A1A1AA] text- mb-6">Lägg till lån och se hur extra betalning påverkar. Big.js exakt. Inget sparas online.</p>

        {isDev && <button onClick={() => setLoans(TEST_LOANS)} className="mb-4 bg-purple-600 px-4 py-2 rounded text-sm">test-lån-fredde</button>}

        {loans.length===0 && <div className="bg-[#18181B] border border-dashed border-[#27272A] p-8 text-center"><div className="text-[#71717A] mb-3">Inga lån tillagda - helt tom prod</div><button onClick={addLoan} className="bg-[#3B82F6] px-6 py-2 rounded text-sm">+ Lägg till ditt första lån</button></div>}
              ...
              return (
                <div key={loan.id} className="bg-[#18181B] border border-[#27272A] p-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
                    <label className="text-"><div className="text-[#71717A] mb-1">Skuld</div><input type="number" min={0} max={9999999} value={loan.balance} onChange={e=>updateLoan(loan.id,{balance:Number(e.target.value)})} className="w-full bg-[#09090B] border border-[#27272A] p- text-" /></label>
                    <label className="text-"><div className="text-[#71717A] mb-1">Ränta %</div><input type="number" step="0.01" value={loan.interestRate*100} onChange={e=>updateLoan(loan.id,{interestRate:Number(e.target.value)/100})} className="w-full bg-[#09090B] border border-[#27272A] p- text-" /></label>
                    <label className="text-"><div className="text-[#71717A] mb-1">{loan.paymentStyle==="fixed_amort"?"Fast amort":"Mån betalning"}</div><input type="number" value={loan.currentMonthlyPayment} onChange={e=>updateLoan(loan.id,{currentMonthlyPayment:Number(e.target.value)})} className="w-full bg-[#09090B] border border-[#27272A] p- text-" /></label>
                  </div>
                  ...
                  <div className="bg-[#09090B] border border-[#27272A] p-3 mb-3">
                    ...
                    {loan.targetMonthlyEnabled? (
                      <div>
                        <div className="grid grid-cols-2 gap-3"><div><div className="text- text-[#71717A]">Min totala månadskostnad</div><input type="number" min={0} max={99999} placeholder="2500" value={loan.targetMonthlyTotal||""} onChange={e=>updateLoan(loan.id,{targetMonthlyTotal:Number(e.target.value)})} className="w-full bg-[#18181B] border border-[#27272A] p-2 text-" /></div><div><div className="text- text-[#71717A]">Från</div><input type="month" value={loan.targetMonthlyFrom||DEFAULT_START} onChange={e=>updateLoan(loan.id,{targetMonthlyFrom:e.target.value})} className="w-full bg-[#18181B] border border-[#27272A] p-2 text-" /></div></div>
                        <div className="mt-2 text- text-[#A1A1AA]">Av <span className="text-white font-mono">{newTotal.toFixed(0)}:-</span> är <span className="text-white">{interestPart.toFixed(0)}:- ränta</span> + <span className="text-[#22C55E]">{newAmort.toFixed(0)}:- amortering</span> • Idag betalar du {loan.currentMonthlyPayment.toFixed(0)}:- ({baseAmort.toFixed(0)}:- amort)</div>
                      </div>
                  ...
                  <div className="bg-[#09090B] border border-[#27272A] p-3">
                    ...
                    <div className="flex gap-2 mb-2"><input type="month" defaultValue={DEFAULT_START} id={`ex-date-${loan.id}`} className="bg-[#18181B] border border-[#27272A] p-2 text- flex-1" /><input type="text" placeholder="Skatteåterbäring" id={`ex-label-${loan.id}`} className="bg-[#18181B] border border-[#27272A] p-2 text- flex-1" /><input type="number" placeholder="15000" id={`ex-amt-${loan.id}`} className="bg-[#18181B] border border-[#27272A] p-2 text- w-24" /><button onClick={()=>{const d=(document.getElementById(`ex-date-${loan.id}`) as HTMLInputElement).value; const lab=(document.getElementById(`ex-label-${loan.id}`) as HTMLInputElement).value; const a=Number((document.getElementById(`ex-amt-${loan.id}`) as HTMLInputElement).value); if(d&&a) setOneTimes(p=>[...p,{id:Date.now().toString(), date:d, amount:a, loanId:loan.id, label:lab}] as any);}} className="bg-[#3B82F6] px-3 text-">+ Lägg till</button></div>
                    <div className="space-y-1">{oneTimes.filter(ot=>ot.loanId===loan.id).map(ot=><div key={ot.id} className="flex justify-between text- bg-[#18181B] p-2"><span>{ot.date} • {(ot as any).label||"Extra"} • {ot.amount}:- </span><div className="flex gap-2"><button onClick={()=>{const copy={...ot, id:Date.now().toString()}; setOneTimes(p=>[...p,copy]);}} className="text-[#3B82F6]">kopiera</button><button onClick={()=>setOneTimes(p=>p.filter(x=>x.id!==ot.id))} className="text-red-400">x</button></div></div>)}</div>
                    {lr && <div className="mt-3 text- text-[#71717A] flex gap-3"><span>Orig ränta: {lr.originalTotalInterest.toLocaleString()}:-</span><span>Ny: {lr.newTotalInterest.toLocaleString()}:-</span><span className="text-[#22C55E]">Sparad: {lr.interestSaved.toLocaleString()}:-</span></div>}
                  </div>
                </div>
              )
            })}
            {loans.length>0 && <button onClick={addLoan} className="w-full bg-zinc-800 border border-[#27272A] py-3 text-sm">+ Lägg till lån</button>}
          </div>
          <div className="space-y-4 lg:sticky lg:top-6 h-fit">
            {result? <><div className="grid grid-cols-2 gap-3"><div className="bg-[#18181B] border border-[#27272A] p-4"><div className="text- text-[#71717A]">SKULDFRI</div><div className="text-xl font-mono">{result.newFreedomDate}</div><div className="text-xs text-[#22C55E]">{result.totalMonthsSaved} mån tidigare</div></div><div className="bg-[#18181B] border border-[#27272A] p-4"><div className="text- text-[#71717A]">SPARAD RÄNTA</div><div className="text-xl font-mono">{result.totalInterestSaved.toLocaleString()}:-</div></div></div></> : <div className="bg-[#18181B] border border-[#27272A] p-4 text-sm text-[#71717A]">Lägg till lån för att se resultat</div>}
          </div>
        </div>
      </div>
    </div>
