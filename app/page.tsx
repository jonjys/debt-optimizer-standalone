...
export default function Page() {
  ...
  const bi = useMemo(() => {
    const bake = Math.min(Math.max(0, biBake), biPersonal);
    const personalLeft = biPersonal - bake;
    const newMortgage = biMortgage + bake;
    const ltvBefore = biHome > 0 ? biMortgage / biHome : 0;
    const ltvAfter = biHome > 0 ? newMortgage / biHome : 0;
    const amortPct = (ltv: number) => (ltv > 0.7 ? 0.02 : ltv > 0.5 ? 0.01 : 0);
    const ab = amortPct(ltvBefore), aa = amortPct(ltvAfter);
    const mr = biMortRate / 100, pr = biPersRate / 100;
    const ann = (bal: number, rate: number) => {
      if (bal <= 0) return 0;
      const r = rate / 12, n = 120;
      return (bal * r * Math.pow(1 + r, n)) / (Math.pow(1 + r, n) - 1);
    };
    const monthBefore = (biMortgage * ab) / 12 + (biMortgage * mr) / 12 + ann(biPersonal, pr);
    const monthAfter = (newMortgage * aa) / 12 + (newMortgage * mr) / 12 + ann(personalLeft, pr);
    const years = 30;
    const mortIntB = ((biMortgage + biMortgage * (1 - ab * years)) / 2) * mr * years;
    const mortIntA = ((newMortgage + newMortgage * (1 - aa * years)) / 2) * mr * years;
    const pB = ann(biPersonal, pr) * 120 - biPersonal;
    const pA = ann(personalLeft, pr) * 120 - personalLeft;
    const totalIntBefore = Math.max(0, mortIntB) + Math.max(0, pB);
    const totalIntAfter = Math.max(0, mortIntA) + Math.max(0, pA);
    return { bake, ltvBefore, ltvAfter, amortBefore: ab, amortAfter: aa, monthBefore, monthAfter, totalIntBefore, totalIntAfter, interestSaved: totalIntBefore - totalIntAfter, warningLtv: ltvAfter > 0.85, netSave: (totalIntBefore - totalIntAfter) * 0.7 };
      ...
      <div className="flex min-h-screen">
        ...
        <main className="flex-1 min-w-0">
          <header className="border-b border-white/[0.06] px-4 sm:px-6 py-3 flex flex-wrap items-center justify-between gap-3">
            ...
            <span className="font-mono text-[12px] text-zinc-400">{loans.length} lån · {fmt(totalDebt)}:-</span>
          </header>
          <div className="px-4 sm:px-6 pt-4 flex gap-1.5 flex-wrap">
            {([["idag", "Idag"], ["jamfor", "Jämför"], ["bakain", "Baka in"]] as const).map(([k, label]) => (
              <button key={k} onClick={() => setTab(k)} className={`text-[12px] px-3 py-1.5 rounded-lg border ${tab === k ? "bg-emerald-600/20 border-emerald-500/40 text-emerald-300 font-semibold" : "border-white/10 text-zinc-500"}`}>{label}</button>
            ))}
          </div>
              ...
              <div className="grid lg:grid-cols-2 gap-5 max-w-4xl">
                <div className="rounded-2xl border border-white/[0.08] bg-[#121216] p-5 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-[12px]">
                    <label><span className="text-zinc-500 text-[10px]">Bolån kr</span><input type="number" value={biMortgage} onChange={(e) => setBiMortgage(+e.target.value || 0)} className="w-full mt-0.5 bg-black/40 border border-white/10 rounded-lg px-2 py-1.5 font-mono" /></label>
                    <label><span className="text-zinc-500 text-[10px]">Bolåneränta %</span><input type="number" step="0.01" value={biMortRate} onChange={(e) => setBiMortRate(+e.target.value || 0)} className="w-full mt-0.5 bg-black/40 border border-white/10 rounded-lg px-2 py-1.5 font-mono" /></label>
                    <label><span className="text-zinc-500 text-[10px]">Blancolån kr</span><input type="number" value={biPersonal} onChange={(e) => setBiPersonal(+e.target.value || 0)} className="w-full mt-0.5 bg-black/40 border border-white/10 rounded-lg px-2 py-1.5 font-mono" /></label>
                    <label><span className="text-zinc-500 text-[10px]">Blanco-ränta %</span><input type="number" step="0.01" value={biPersRate} onChange={(e) => setBiPersRate(+e.target.value || 0)} className="w-full mt-0.5 bg-black/40 border border-white/10 rounded-lg px-2 py-1.5 font-mono" /></label>
                    <label className="col-span-2"><span className="text-zinc-500 text-[10px]">Bostadsvärde kr</span><input type="number" value={biHome} onChange={(e) => setBiHome(+e.target.value || 0)} className="w-full mt-0.5 bg-black/40 border border-white/10 rounded-lg px-2 py-1.5 font-mono" /></label>
                  </div>
                  <div>
                    <div className="flex justify-between text-[11px] mb-1"><span className="text-zinc-400">Baka in</span><span className="font-mono text-emerald-400 font-bold">{fmt(biBake)} kr</span></div>
                ...
                <div className="rounded-2xl border border-white/[0.08] bg-[#121216] p-5 space-y-3">
                  <h2 className="text-sm font-bold">Resultat</h2>
                  {bi.warningLtv && <div className="text-[11px] text-amber-300 bg-amber-500/10 border border-amber-500/30 rounded-lg px-3 py-2">Varning: LTV {(bi.ltvAfter * 100).toFixed(0)}% &gt; 85%</div>}
                  <div className="grid grid-cols-2 gap-2 text-[11px]">
                    <div className="rounded-lg bg-black/30 p-3"><div className="text-zinc-500 text-[9px] uppercase mb-1">Belåningsgrad</div><div className="font-mono">{(bi.ltvBefore * 100).toFixed(0)}% → {(bi.ltvAfter * 100).toFixed(0)}%</div></div>
                    <div className="rounded-lg bg-black/30 p-3"><div className="text-zinc-500 text-[9px] uppercase mb-1">Amorteringskrav</div><div className="font-mono">{(bi.amortBefore * 100).toFixed(0)}% → {(bi.amortAfter * 100).toFixed(0)}%/år</div></div>
                    <div className="rounded-lg bg-black/30 p-3"><div className="text-zinc-500 text-[9px] uppercase mb-1">Månadskostnad</div><div className="font-mono">{fmt(bi.monthBefore)} → {fmt(bi.monthAfter)}:-</div></div>
                    <div className="rounded-lg bg-black/30 p-3"><div className="text-zinc-500 text-[9px] uppercase mb-1">Total ränta</div><div className="font-mono">{fmt(bi.totalIntBefore)} → {fmt(bi.totalIntAfter)}:-</div></div>
                  </div>
                  <div className="rounded-xl border border-emerald-500/30 bg-emerald-950/20 p-4 text-sm">
                    {bi.interestSaved > 0 ? (<p><span className="font-bold text-emerald-300">Baka in {fmt(bi.bake)} kr</span> sparar ca <span className="font-mono font-bold text-emerald-300">{fmt(bi.interestSaved)} kr</span> brutto. Netto (30% avdrag): <span className="font-mono">{fmt(bi.netSave)} kr</span>.</p>) : (<p className="text-zinc-400">Dra slidern för besparing.</p>)}
                  </div>
                </div>
              </div>
            ) : tab === "jamfor" ? (
                ...
                <div className="flex gap-3 overflow-x-auto pb-2">
                  {[{"t": "Idag (din ordning)", "r": resultCustom}, {"t": "Lavin (högst ränta)", "r": resultAva}, {"t": "Snöboll (lägst skuld)", "r": resultSnow}].map((c) => (
                    <div key={c.t} className="rounded-2xl border border-white/[0.08] bg-[#121216] p-4 min-w-[200px] flex-1">
                      ...
                      {c.r && c.r.newFreedomDate !== "-" ? (<><div className="font-mono text-xl font-bold">{c.r.newFreedomDate}</div><div className="text-[11px] text-zinc-400 mt-1 font-mono">Ränta {fmt(c.r.totalNewInterest)}:-</div>{c.r.totalInterestSaved > 0 && <div className="text-[11px] text-emerald-400 mt-1">Sparar {fmt(c.r.totalInterestSaved)}:- · −{c.r.totalMonthsSaved} mån</div>}</>) : (<div className="text-zinc-600 text-sm">—</div>)}
                    </div>
                  ))}
                </div>
                    ...
                    return (
                      <div key={loan.id} className="rounded-2xl border border-white/[0.08] bg-[#121216] p-4 space-y-3">
                        <div className="flex justify-between gap-2">
                          ...
                          <div className="text-right shrink-0"><div className="text-[9px] text-zinc-600">KLAR</div><div className="font-mono text-sm text-emerald-400">{lr?.newEndDate && lr.newEndDate !== "-" ? lr.newEndDate : "—"}</div>{lr && lr.monthsSaved > 0 && <div className="text-[10px] text-emerald-500/80">−{lr.monthsSaved} mån</div>}</div>
                        </div>
                        <div className="grid grid-cols-3 gap-2 text-[11px]">
                          <div className="rounded-lg bg-black/40 p-2"><div className="text-[9px] text-zinc-600">SKULD</div><div className="font-mono font-semibold">{fmt(loan.balance)}:-</div></div>
                          <div className="rounded-lg bg-black/40 p-2"><div className="text-[9px] text-zinc-600">{isRak ? "AMORT" : "MÅN"}</div><div className="font-mono font-semibold">{fmt(loan.currentMonthlyPayment)}:-</div></div>
                          <div className="rounded-lg bg-black/40 p-2"><div className="text-[9px] text-zinc-600">RÄNTA</div><div className="font-mono font-semibold">{fmt(intM)}:-</div></div>
                        </div>
                        ...
                        {lr && <div className="flex gap-3 text-[10px] font-mono text-zinc-500"><span>Orig: {fmt(lr.originalTotalInterest)}:-</span><span>Ny: {fmt(lr.newTotalInterest)}:-</span><span className="text-emerald-400">Sparad: {fmt(lr.interestSaved)}:-</span></div>}
                      </div>
                    );
                  })}
                  {loans.length > 0 && (
                    <div className="rounded-2xl border border-white/[0.08] bg-[#121216] p-3 flex flex-wrap items-center gap-2">
                      ...
                      <input type="number" value={globalExtra || ""} onChange={(e) => setGlobalExtra(+e.target.value || 0)} placeholder="500" className="w-20 bg-black/40 border border-white/10 rounded-lg px-2 py-1 font-mono text-sm" />
                      ...
                      <input type="number" id="otp-amt" placeholder="10000" className="w-20 bg-black/40 border border-white/10 rounded-lg px-2 py-1 font-mono text-sm" />
                      ...
                      {oneTimes.map((ot) => (<span key={ot.id} className="text-[10px] font-mono text-zinc-400 flex items-center gap-1">{ot.date} {fmt(ot.amount)}:- <button onClick={() => setOneTimes((p) => p.filter((x) => x.id !== ot.id))} className="text-red-400">×</button></span>))}
                    </div>
                  )}
                  ...
                  {result && totalDebt > 0 ? (
                    <>
                      <div className="grid grid-cols-2 gap-3">
                        <div className="rounded-2xl border border-white/[0.08] bg-[#121216] p-4"><div className="text-[9px] uppercase text-zinc-500 mb-1">Skuldfri</div><div className="font-mono text-2xl font-bold">{result.newFreedomDate !== "-" ? result.newFreedomDate : "—"}</div>{result.totalMonthsSaved > 0 && <div className="text-[11px] text-emerald-400 mt-1">−{result.totalMonthsSaved} mån snabbare</div>}</div>
                        <div className="rounded-2xl border border-white/[0.08] bg-[#121216] p-4"><div className="text-[9px] uppercase text-zinc-500 mb-1">Sparad ränta</div><div className="font-mono text-2xl font-bold text-emerald-400"><AnimatedNumber value={result.totalInterestSaved} />:-</div><div className="text-[10px] text-zinc-500 mt-1 font-mono">{fmt(result.totalOriginalInterest)} → {fmt(result.totalNewInterest)}</div></div>
                      </div>
                      <div className="rounded-2xl border border-white/[0.08] bg-[#121216] p-4">
                        ...
                        {[...(result.loanResults || [])].sort((a, b) => a.payoffOrder - b.payoffOrder).map((r) => (
                          <div key={r.id} className="flex justify-between text-[12px] py-1"><span className="font-medium">{r.payoffOrder}. {r.name}</span><span className="font-mono text-[11px]"><span className="text-zinc-600">{r.originalEndDate}</span> → <span className="text-emerald-400">{r.newEndDate}</span></span></div>
                        ))}
                      </div>
                      <div className="rounded-2xl border border-white/[0.08] bg-[#121216] p-4">
                        <div className="text-[9px] uppercase text-zinc-500 mb-1">Betalning / mån</div>
                        <div className="font-mono text-lg font-bold">{fmt(totalMin + globalExtra)}:-</div>
